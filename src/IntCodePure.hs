...
data Result n
  = Halted
  | NewOutput n (Continuation n)
  | RequestInput (n -> Continuation n)
  | Error String


newtype Continuation n = Continuation { getComputer :: Computer n }
...
parameterModeFromInt :: (Num n, Eq n, Show n) => MonadError (Result n) m => n -> m ParameterMode
...
parameterModeFromInt m = Except.throwError $ Error $ "invalid parameter-mode " ++ show m
...
--   over STM
type IntCodeM n a = StateT (Computer n) (Except (Result n)) a


initComputer :: (Eq n, Integral n, Show n) => Program n -> Continuation n
initComputer = Continuation . createComputer


runComputer :: (Eq n, Integral n, Show n) => Continuation n -> Result n
runComputer = stepComputer . getComputer


stepComputer :: (Eq n, Integral n, Show n) => Computer n -> Result n
stepComputer = either id (const Halted) . Except.runExcept . State.evalStateT executeProgram


createComputer :: (Integral n, Show n, Eq n) => Program n -> Computer n
createComputer prg =
  Computer (Map.fromList $ zip [0..] prg) 0 0 Nothing
...
runInstruction (Output from) = do
  ...
  -- move next so the computer will continue after the output
  moveNext
  writeOutput out
...
getInstruction address = do
  ...
  case opCode of
    ...
    _ -> Except.throwError $ Error $ "unknown opcode " ++ show opCode
  where
...
evalAddress (Parameter ImmediateMode _) =
  Except.throwError $ Error "Immediate-Mode in Addressing"
...
getMemory adr | adr < 0 =
  Except.throwError $ Error $ "tried to access the negative memory address " ++ show adr
getMemory adr = do
...
-- | get's the next user provided input if there are any
--   if not interrupts the program requesting it
readInput :: Show n => IntCodeM n n
readInput = do
  input <- State.gets nextInput
  State.modify' (\s -> s { nextInput = Nothing })
  case input of
    Just n -> pure n
    Nothing -> do
      prg <- State.get
      Except.throwError $ RequestInput (\inp -> Continuation ( prg { nextInput = Just inp }))


-- | interrupts the program with an output
writeOutput :: Show n => n -> IntCodeM n ()
writeOutput out = do
  prg <- State.get
  Except.throwError $ NewOutput out (Continuation prg)
